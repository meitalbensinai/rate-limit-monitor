...
#
# Safe and idempotent: backs up settings.json, then wires the statusline and the
# UserPromptSubmit guard WITHOUT removing any existing statusline or hooks.
# Re-running it changes nothing. Works from wherever this repo lives (the wired
# paths point back at this checkout).
#
...
chmod +x "$GUARD" "$SL_SCRIPT" 2>/dev/null || true
mkdir -p "$CONFIG_DIR"
...
# --- 2. statusline (chain any existing one; skip if already ours) ------------
...
if [ "$changed" -eq 1 ]; then
  echo "Done. RESTART Claude Code to load the statusline + guard."
else
  echo "Already installed — nothing to change."
fi
echo "Tune with env vars (see README): RLM_THRESHOLD, RLM_WINDOWS, RLM_OVERRIDE."
